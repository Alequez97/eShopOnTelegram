...
export function useCartItemsState() {
	...
	const addProductAttributeToState = (productAttribute: ProductAttribute) => {
		...
		if (existingCartItem) {
            const newState = cartItems.map((cartItem) => {
        ...
        const newState = [...cartItems, { productAttribute, quantity: 1 }];

		setCartItems(newState);
	};

	const removeProductAttributeFromState = (
		...
		if (existingCartItem?.quantity === 1) {
            const newState = cartItems.filter(
				(cartItem) =>
